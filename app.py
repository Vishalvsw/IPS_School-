...
def register_student():
    try:
        ...
        # List of required fields based on your database schema (excluding auto-generated/optional ones)
        required_fields = [
            'name', 'father_name', 'dob', 'class', 'gender', 'academic_year',
            'aadhar', 'village', 'taluka', 'district', 'pin', 'religion',
            'caste', 'nationality', 'mother_tongue', 'medium', 'residential_address',
            'phone1', 'payment_utr', 'exam_center'
        ]

        # Validate required form fields
        for field in required_fields:
            if not student_data.get(field):
                flash(f"'{field.replace('_', ' ').title()}' is a required field.", "danger")
                return redirect(url_for('register'))

        # Validate file uploads and ensure they are not None
        if not photo:
            flash("Photo must be uploaded.", "danger")
            return redirect(url_for('register'))
        if not signature:
            flash("Signature must be uploaded.", "danger")
            return redirect(url_for('register'))

        # Assert that filenames are not None for type checking purposes
        assert photo.filename is not None, "Photo filename cannot be None here."
        assert signature.filename is not None, "Signature filename cannot be None here."
